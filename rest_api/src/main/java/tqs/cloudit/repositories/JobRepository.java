...
public interface JobRepository extends CrudRepository<Job, Long> {
    ...
    @Query(value="select * from job where area like concat('%',?1,'%') and amount >= ?2 and amount <= ?3 and date >= ?4 and date <= ?5 and type='Offer' and not finished and worker is not null", nativeQuery = true)
    ...
    @Query(value="SELECT * FROM job WHERE amount >= ?1 AND amount <= ?2 AND date >= ?3 AND date <= ?4 AND NOT finished AND worker IS NOT NULL type='Offer'", nativeQuery = true)
